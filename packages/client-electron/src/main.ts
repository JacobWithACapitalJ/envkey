...
import { initFileLogger, log } from "@envkey/core/lib/utils/logger";
import url from "url";
import { app, screen, BrowserWindow, Menu, MenuItem, ipcMain, dialog } from "electron";
...
app.on("ready", () => {
  ...
  ipcMain.handle("dialog-show-save", async (_e, opts: { title: string; defaultPath: string }) => {
    const { filePath } = await dialog.showSaveDialog(opts);
    return filePath;
  });

  ipcMain.handle("dialog-show-open-file", async (_e, opts: { message: string; filters?: Electron.FileFilter[] }) => {
  ...
  ipcMain.handle("dialog-show-open-dir", async (_e, opts: { message: string }) => {
    const { filePaths } = await dialog.showOpenDialog({
      message: opts.message,
